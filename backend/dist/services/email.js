...
class EmailService {
    ...
    initialize() {
        ...
        if (sendgridApiKey) {
            ...
            logger_js_1.default.info("Email service initialized with SendGrid");
        }
        else if (brevoApiKey) {
            this.provider = "brevo";
            this.config = {
            ...
            logger_js_1.default.info("Email service initialized with Brevo");
        }
        else {
            logger_js_1.default.warn("No email provider configured. Email sending disabled.");
        }
    }
    async sendCertificateEmail(to, recipientName, certificateId, courseName, pdfBase64) {
        if (!this.config) {
            logger_js_1.default.warn("Email service not configured");
            return false;
        }
    ...
    async sendWithSendGrid(options) {
        ...
        logger_js_1.default.info(`Email sent to ${options.to}: ${options.subject}`);
        return true;
    }
    async sendWithBrevo(options) {
        ...
        logger_js_1.default.info(`Email sent to ${options.to} via Brevo: ${options.subject}`);
        return true;
    }
}
