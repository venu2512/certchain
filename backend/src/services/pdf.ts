...
const generatePDF = async (data: CertificateData): Promise<Buffer> => {
  const doc = new jsPDF({
  ...
  doc.text(
    "This certificate can be verified at: certchain.vercel.app/verify",
    centerX,
    pageHeight - 15,
    { align: "center" }
  );
  ...
  doc.setFontSize(6);
  doc.text(
    `Generated: ${new Date().toISOString()} | CertChain v2.0`,
    centerX,
    pageHeight - 10,
    { align: "center" }
  );
...
export const generateCertificatePDF = async (data: CertificateData): Promise<Buffer> => {
  try {
    logger.info(`Generating PDF for certificate: ${data.certificateId}`);
